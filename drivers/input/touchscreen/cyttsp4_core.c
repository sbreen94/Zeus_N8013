			...
			struct device_attribute *attr, char *buf)
{
	...
	command_buf[5] = 0x09;
	rc = cyttsp4_exec_cmd_(cd, CY_MODE_CAT,
			command_buf, 6, return_buf, 7, 500);
	if (rc < 0) {
...
exit:
	...
	menu_sensitivity_tmp = return_buf[5];
	menu_sensitivity_tmp += return_buf[6] << 8;
	/* unsigned -> signed */
			...
			struct device_attribute *attr, char *buf)
{
	...
	command_buf[5] = 0x09;
	rc = cyttsp4_exec_cmd_(cd, CY_MODE_CAT,
			command_buf, 6, return_buf, 15, 500);
	if (rc < 0) {
		dev_err(cd->dev, "%s: fail to execute retrieve panel scan command\n",
			__func__);
		goto exit_switch;
...
exit:
	...
	back_sensitivity_tmp = return_buf[13];
	back_sensitivity_tmp += return_buf[14] << 8;
	/* unsigned -> signed */
